...
float ringstep = 0; // 环岛状态变量
float parkingstep = 0; // 停车场状态变量
float stdevLeft = 0; // 左边缘标准差
float stdevRight = 0; // 右边缘标准差
...
int main(int argc, char const *argv[]) {
  ...
  captureInterface.Start();  // 开启图像采集线程


  uart->buzzerSound(uart->BUZZER_OK); // 祖传提示音效
  for(int i = 0;i<250;i++)
  {
      uart->sendKd2(motion.params.kd2); // 发送PID参数
  }
  std::cout << "kd2: " << motion.params.kd2 << std::endl;
  ...
  std::ofstream file;
  file.open("/home/edgeboard/Desktop/sasu2025yz5/src/src/text.txt", std::ios::trunc);
  file.close();
  ofstream file_pid;
  file_pid.open("/home/edgeboard/Desktop/sasu2025yz5/src/src/pid.txt",std::ios::trunc);
  file_pid.close();
  ...
  while (1) {
    ...
    img = captureInterface.get_frame();//使用异步线程采图
    ...
    stdevLeft = tracking.stdevLeft; // 左边缘标准差
    stdevRight = tracking.stdevRight; // 右边缘标准差
    std::cout << "stdevLeft: " << stdevLeft << std::endl;
    std::cout << "stdevRight: " << stdevRight << std::endl;
    ...
    if (motion.params.stop) {
      if(crosswalk.process(detection._predictor->results))//通过分析图像特征判断当前是否处于停车区中
      {
        scene = Scene::StopScene;               
            if (crosswalk.countExit > 6) 
            {
    ...
    if ((scene == Scene::NormalScene || scene == Scene::CateringScene) &&
      ...
      if (catering.process(tracking,imgBinary,detection._predictor->results))  // 传入二值化图像进行再处理
    ...
    if ((scene == Scene::NormalScene || scene == Scene::LaybyScene) &&
        motion.params.catering) {
      if (layby.process(tracking,imgBinary,detection._predictor->results))  // 传入二值化图像进行再处理
    ...
    if ((scene == Scene::NormalScene || scene == Scene::ParkingScene) &&
      ...
      if (parking.process(tracking,imgBinary,detection._predictor->results))  // 传入二值化图像进行再处理
      {
        scene = Scene::ParkingScene;
        parkingstep = parking.step;
      }
      else
        scene = Scene::NormalScene;
    } 
   ...
   // [09] 障碍区检测
    if ((scene == Scene::NormalScene || scene == Scene::ObstacleScene) &&
      ...
      if (obstacle.process(tracking, detection._predictor->results)) {
...
/*
   //[15] 综合显示调试UI窗口
    if (1) {
      ...
      case Scene::StopScene:    //[ 斑马线 ]
        crosswalk.drawImage(imgRes); // 图像绘制特殊赛道识别结果
        circle(imgCorrect, Point(COLSIMAGE / 2, ROWSIMAGE / 2), 40,
               Scalar(40, 120, 250), -1);
        putText(imgCorrect, "E", Point(COLSIMAGE / 2 - 25, ROWSIMAGE / 2 + 27),FONT_HERSHEY_PLAIN, 5, Scalar(255, 255, 255), 3);
...
*/
    ...
    sceneLast = scene; // 记录当前状态 状态机的 “复位逻辑” 作用是:在每次循环结束时将特殊场景状态重置回普通状态（Scene::NormalScene），这样下一轮检测可以重新判断场景类型
