...
public:
    ...
    void fitting(Tracking &track)
    {
       ...
       // 通过双边缘有效点的差来判断赛道类型，使用双段三阶贝塞尔拟合中线
        ...
        ///////////////////////////////////////////// 
        // === 判断：中心行的中线偏移 ===
        bool isRightCurve = false;
        bool isLeftCurve = false;

        if (centerEdge.size() > 10) {
            int midRow = ROWSIMAGE / 2;
            int minDiff = ROWSIMAGE;
            int midCenterY = COLSIMAGE / 2;

            for (auto &p : centerEdge) {
                int diff = abs(p.x - midRow);
                if (diff < minDiff) {
                    minDiff = diff;
                    midCenterY = p.y;
                }
        ...
        for (auto p : centerEdge)
        {
            if(isRightCurve == true)
            {
                ...
                else if(p.x < ROWSIMAGE && p.x >= ROWSIMAGE * 4 / 5)
                {
                    double temp = normal_pdf(curve_fitting_output(ROWSIMAGE - p.x), miu, singema);
                    controlNum += temp * 0.6;
                    controlCenter_Calculate += p.y * temp * 0.6;
                }
