...
// 计算目标段的最小二乘斜率
    double LeastSquare_2(const std::vector<POINT>& points, size_t start, size_t end)
    {
        if (start >= end || end > points.size())
        {
            std::cerr << "LeastSquare: 无效的拟合范围" << std::endl;
            return 0;
        }

        size_t n = end - start;
        double sum_y = 0, sum_x = 0, sum_y2 = 0, sum_yx = 0;

        for (size_t i = start; i < end; ++i)
        {
            double x = points[i].x;  // 垂直方向
            double y = points[i].y;  // 水平方向（当作自变量）
        ...
        double denominator = n * sum_y2 - sum_y * sum_y;
        if (std::fabs(denominator) < 1e-6)
        {
            std::cerr << "LeastSquare: 拟合失败，可能是竖直线或点分布异常" << std::endl;
            return 0;
        }

        double k = (n * sum_yx - sum_y * sum_x) / denominator;
        return k;  // 代表：图像中线段“从左往右”时的上升/下降趋势
    }
	 ...
	 * @return vector<POINT>
	 */
	vector<POINT> predictEdgeLeft(vector<POINT> pointsEdgeRight, bool bezier = true, int offset = 160)
	{
		// int offset = 120; // 右边缘平移尺度
    ...
    vector<POINT> predictEdgeLeft_2(vector<POINT> pointsEdgeRight, bool bezier = true, int base_offset = 160)
{
    vector<POINT> pointsEdgeLeft;
    if (pointsEdgeRight.size() < 3)
        return pointsEdgeLeft;
    ...
    if (!bezier)
    {
        for (int i = 1; i < pointsEdgeRight.size(); i++)
        {
            // 1. 当前点透视变换
            Point2d pt_curr = ipm.homography(Point2d(pointsEdgeRight[i].y, pointsEdgeRight[i].x));
            Point2d pt_prev = ipm.homography(Point2d(pointsEdgeRight[i - 1].y, pointsEdgeRight[i - 1].x));
            ...
            // 3. 计算动态偏移（适应曲线）
            double dynamic_offset = base_offset + 40.0 * sin(angle); // 可调：40.0

            // 4. 左移并反透视
            Point2d predictPt = Point2d(pt_curr.x - dynamic_offset, pt_curr.y);
            Point2d pt_image = ipm.homographyInv(predictPt);
            POINT edgePoint(pt_image.y, pt_image.x);

            // 5. 边界处理
            edgePoint.x = std::clamp(edgePoint.x, 0, ROWSIMAGE - 1);
            edgePoint.y = std::clamp(edgePoint.y, 0, COLSIMAGE - 1);
    ...
    // 对于贝塞尔版本，仅取 3 点进行处理（首中尾）

    auto get_dynamic_left = [&](int idx) -> POINT {
        Point2d pt_curr = ipm.homography(Point2d(pointsEdgeRight[idx].y, pointsEdgeRight[idx].x));
        Point2d pt_prev = ipm.homography(Point2d(pointsEdgeRight[std::max(0, idx - 1)].y,
                                                 pointsEdgeRight[std::max(0, idx - 1)].x));
        double dx = pt_curr.x - pt_prev.x;
        double dy = pt_curr.y - pt_prev.y;
        double angle = atan2(dy, dx);
        double dynamic_offset = base_offset + 40.0 * sin(angle);

        Point2d predictPt = Point2d(pt_curr.x - dynamic_offset, pt_curr.y);
        Point2d pt_image = ipm.homographyInv(predictPt);
        return POINT(pt_image.y, pt_image.x);
    };

    POINT startPoint = get_dynamic_left(0);
    POINT midPoint = get_dynamic_left(pointsEdgeRight.size() / 2);
    POINT endPoint = get_dynamic_left(pointsEdgeRight.size() - 1);
    ...
    for (POINT &p : repair)
    {
        p.x = std::clamp(p.x, 0, ROWSIMAGE - 1);
        p.y = std::clamp(p.y, 0, COLSIMAGE - 1);
        pointsEdgeLeft.push_back(p);
    }
