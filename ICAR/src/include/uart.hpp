...
#define USB_ADDR_KD2 7     //舵机pid的kd2值
#define USB_ADDR_CATER_ENTER 9   //陀螺仪积累转过的角度
#define USB_ADDR_RING_ENTER 2 //环岛状态：进入
#define USB_ADDR_RING_INSIDE 3 //环岛状态：环中
#define USB_ADDR_RING_FINISH 8 //环岛状态：出环
...
public:
  ...
  int open(void) {
    ...
    try {
      ...
      serialPort->SetBaudRate(BaudRate::BAUD_500000); // 设置波特率
  ...
  void dataTransform(void) {
    ...
    case USB_ADDR_CATER_ENTER:
          Bit32Union bit32U;  // Uart 类内定义的联合体，便于将4字节转换为 float 数据
          // 从接收缓冲区复制4字节数据到联合体中
          for (int i = 0; i < 4; i++) {
              bit32U.buff[i] = serialStr.buffFinish[3 + i];
          }
          cater_enter = bit32U.float32;
          
          // 可在此处进行后续处理，如保存、显示或传递到上层逻辑
          //std::cout << "Received angle: " << angle << std::endl;
          break;

    case USB_ADDR_RING_ENTER:
            Bit32Union bit32U_enter;  // Uart 类内定义的联合体，便于将4字节转换为 float 数据
            // 从接收缓冲区复制4字节数据到联合体中
            for (int i = 0; i < 4; i++) {
                bit32U_enter.buff[i] = serialStr.buffFinish[3 + i];
            }
            ring_enter = bit32U_enter.float32;
            
            // 可在此处进行后续处理，如保存、显示或传递到上层逻辑
            std::cout << "ring_enter: " << ring_enter << std::endl;
            break;

    case USB_ADDR_RING_INSIDE:
            Bit32Union bit32U_inside;  // Uart 类内定义的联合体，便于将4字节转换为 float 数据
            // 从接收缓冲区复制4字节数据到联合体中
            for (int i = 0; i < 4; i++) {
                bit32U_inside.buff[i] = serialStr.buffFinish[3 + i];
            }
            ring_inside = bit32U_inside.float32;
            
            // 可在此处进行后续处理，如保存、显示或传递到上层逻辑
            std::cout << "ring_inside: " << ring_inside << std::endl;
            break;      
     
    case USB_ADDR_RING_FINISH:
            Bit32Union bit32U_finish;  // Uart 类内定义的联合体，便于将4字节转换为 float 数据
            // 从接收缓冲区复制4字节数据到联合体中
            for (int i = 0; i < 4; i++) {
                bit32U_finish.buff[i] = serialStr.buffFinish[3 + i];
            }
            ring_finish = bit32U_finish.float32;
            
            // 可在此处进行后续处理，如保存、显示或传递到上层逻辑
            std::cout << "ring_finish: " << ring_finish << std::endl;
            break;        
    default:
      break;
    }
...
public:
 //向下位机发送舵机pid的kd2值
  void sendKd2(float kd2) {
  uint8_t buff[8];
  uint8_t check = 0;
  Bit32Union bit32U; // 使用 Uart 类内定义的32位联合体

  buff[0] = USB_FRAME_HEAD;  // 帧头
  buff[1] = USB_ADDR_KD2;    // 地址为 USB_ADDR_KD2
  buff[2] = 8;             // 帧长：8字节
  ...
  void send_cater_enter(float cater_enter) {
  uint8_t buff[8];
  uint8_t check = 0;
  Bit32Union bit32U; // 使用 Uart 类内定义的32位联合体

  buff[0] = USB_FRAME_HEAD;  // 帧头
  buff[1] = USB_ADDR_CATER_ENTER;    // 地址为 USB_ADDR_CATER_ENTER
  buff[2] = 8;             // 帧长：8字节

  // 将 float 数据转换为4字节
  bit32U.float32 = cater_enter;
  for (int i = 0; i < 4; i++) {
      buff[3 + i] = bit32U.buff[i];
  }
  ...
  void send_ring_enter(float ring_enter) {
  uint8_t buff[8];
  uint8_t check = 0;
  Bit32Union bit32U; // 使用 Uart 类内定义的32位联合体

  buff[0] = USB_FRAME_HEAD;  // 帧头
  buff[1] = USB_ADDR_RING_ENTER;    // 地址为 USB_ADDR_KD2
  buff[2] = 8;             // 帧长：8字节

  // 将 float 数据转换为4字节
  bit32U.float32 = ring_enter;
  for (int i = 0; i < 4; i++) {
      buff[3 + i] = bit32U.buff[i];
  }
  ...
  void send_ring_inside(float ring_inside) {
  uint8_t buff[8];
  uint8_t check = 0;
  Bit32Union bit32U; // 使用 Uart 类内定义的32位联合体

  buff[0] = USB_FRAME_HEAD;  // 帧头
  buff[1] = USB_ADDR_RING_INSIDE;    // 地址为 USB_ADDR_KD2
  buff[2] = 8;             // 帧长：8字节

  // 将 float 数据转换为4字节
  bit32U.float32 = ring_inside;
  for (int i = 0; i < 4; i++) {
      buff[3 + i] = bit32U.buff[i];
  }
  ...
  void send_ring_finish(float ring_finish) {
  uint8_t buff[8];
  uint8_t check = 0;
  Bit32Union bit32U; // 使用 Uart 类内定义的32位联合体

  buff[0] = USB_FRAME_HEAD;  // 帧头
  buff[1] = USB_ADDR_RING_FINISH;    // 地址为 USB_ADDR_KD2
  buff[2] = 8;             // 帧长：8字节

  // 将 float 数据转换为4字节
  bit32U.float32 = ring_finish;
  for (int i = 0; i < 4; i++) {
      buff[3 + i] = bit32U.buff[i];
  }
