...
public:
    ...
    {
        _loop = true;
        _thread = std::make_unique<std::thread>(&CaptureInterface::run, this); //启动线程
    }
    ...
    void run()
    {
        ...
        if (!_cap || !_cap->isOpened()) {
            std::cout << "Error: Camera is not opened properly!" << std::endl;
            return;
        }
        ...
        _cap->set(cv::CAP_PROP_EXPOSURE, -7);//曝光等级
    ...
    cv::Mat get_frame(void)
    {
        ...
        while(_frame == nullptr)
        {
            if(cond_.wait_for(lock, std::chrono::seconds(1)) == std::cv_status::timeout) {
                std::cout << "get_frame timeout!" << std::endl;
                return cv::Mat();//返回空图像
            }
        }
