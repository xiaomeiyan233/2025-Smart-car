...
signed short gyrox,gyroy;          //陀螺仪原始数据
...
*************************************************************************/
void STM0_CH0_IRQHandler(void)//1ms进一次中断
{
    static char count1 = 0,count2 = 0;
	/* 开启CPU中断  否则中断不可嵌套 */
	...
	if(count2 >= 10)
	{
        MPU_Get_Raw_data(&aacx,&aacy,&aacz,&gyrox,&gyroy,&gyroz);   //得到陀螺仪传感器数据
        gyroz_s = gyroz / 16.4f;
        if(gyroz_s <= 0.5 && gyroz_s >= -0.5)
        {
            gyroz_s = 0;
        }
        angle_sum = angle_sum + gyroz_s * 0.01;
        count2 = 0;
	}
	/* 用户代码 */
    GPIO_Timer();            //GPIO外设线程
    ...
    Timer_Key_Read();        //记录按键按下时间
    if(count1>=20 && Key_Number==0)
    {
        count1 = 0;
        Key_Number = Key_Scan();  //按键扫描函数，按键按下再弹起计数
    }
